...
////////////////////////////////////////////////////////////////////////Function to Load the ROLES/////////////////////////////////
    function load_roles(data){
        ...
        for(i in data.volunteers){
            var button_registration =$('<a>').attr({href:'', class:'volunteer_unregistered', 'data-ref-role':data.volunteers[i].id}).append(
                $('<img>').attr({src:not_registered_button}));
            for (j in data.volunteers[i].volunteers_registration){
                ...
                if(profile_id === user_id ){
                    button_registration = $('<a>').attr({href:data.volunteers[i].volunteers_registration[j].url, class:'volunteer_registered', 'data-ref-role':data.volunteers[i].id}).append(
            ...
            $('#roles').append(
                $('<li>').css('margin-bottom','5px').append(
                    $('<div>').attr('class','row').css({display:'flex', 'align-items':'center'}).append(
                        ...
                        $('<div>').attr({'class':'col-4','id':'check_'+data.volunteers[i].id }).append(button_registration
...
//////////////////////////////////////////////////////Function LOAD ATTENDEES INFO/////////////////////////////////////////////////////////////////////////////
    function load_attendees(data){
        ...
        for(i in data.attendees.attendees_registration){
            ...
            var button_registration;
            if(profile_id === user_id ){
                $('#unregistration_attendees').attr({'data-ref-role':data.attendees.id,'href': data.attendees.attendees_registration[i].url}).show();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $.getJSON( link, function( data ){
        if(data.user_Is_Owner === true){ //if user is organizer show the edit and delete button
            ...
            ).append($('<a>').attr({href:data.url,class:"btn btn-danger",id:'delete_project'}).css("margin-left","5px").text('Delete'));
        }
